...
canonical JSON shape consumed by the Optimizely Insights deploy agent.

Usage:
    python3 data_to_json.py --kind projects     <input.{json,docx,xlsx}>
    python3 data_to_json.py --kind accelerators <input.{json,docx,xlsx}>
    python3 data_to_json.py --kind quarterly    <input.json>   # JSON passthrough only
...
Canonical shapes (must match data/projects.ts and data/accelerators.ts):
  ...
  accelerators: { org, title, heading, intro, generated,
                  items[] { n, slug, name, category, tagline, maturity, customerValue,
                            ...
                            resolves[], idealCustomer, signals[], valueOptimizely, valueRoyalCyber } }

Field labels/headers are matched case- and punctuation-insensitively, so both
human labels ("Customer Value", "Value for Optimizely", "Challenge Title") and
the canonical camelCase keys ("customerValue", "valueOptimizely", "challengeTitle")
are accepted.
"""
...
ACC_INTRO_DEFAULT = ("Each accelerator extends Optimizely with applied AI, analytics and automation "
                     "— and answers a real customer pain. Built and proven by the Royal Cyber "
                     "Optimizely Center of Excellence, they shorten time-to-value across B2B commerce, "
                     "content management, analytics and digital asset management.")
...
ACC_FIELD_MAP = {
    ...
    "valueoptimizely": "valueOptimizely", "valueforoptimizely": "valueOptimizely",
    "valueroyalcyber": "valueRoyalCyber", "valueforroyalcyber": "valueRoyalCyber",
    "video": "videoUrl", "videourl": "videoUrl", "videotitle": "videoTitle",
}
...
def _build_item(d, idx):
    ...
    item = {
        ...
        "valueOptimizely": clean(d.get("valueOptimizely", "")),
        "valueRoyalCyber": clean(d.get("valueRoyalCyber", "")),
    }
    vurl = clean(d.get("videoUrl", ""))
    if vurl:
        item["video"] = {"url": vurl}
        vtitle = clean(d.get("videoTitle", ""))
        if vtitle:
            item["video"]["title"] = vtitle
    return item
...
def main():
    ...
    ap.add_argument("--kind", required=True, choices=["projects", "accelerators", "quarterly"])
    ...
    if ext == ".json":
        ...
        if a.kind == "quarterly":
            sys.exit("quarterly source must be JSON (.json); .docx/.xlsx are not supported for --kind quarterly")
